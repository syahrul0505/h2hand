        ...
        <div class="card radius-10 bg-danger">
            <div class="card-body">
                <div class="d-flex align-items-center">
                    <div>
                        <p class="mb-0 text-white">Laba Kotor</p>
                        <h4 class="my-1 text-white">Rp. {{ number_format($expenses,0) }}</h4>
                    </div>
...
<div class="col-xl-12 col-lg-12 col-sm-12  layout-spacing">
    ...
    <div class="widget-content widget-content-area br-8">
        <table id="expenses-table" class="table dt-table-hover" style="width:100%">
            <thead>
                <tr>
                    <th width="7%">No</th>
                    <th>Type</th>
...
<script>
    ...
    $("div.toolbar").html('<button class="ms-2 btn btn-primary expenses-add" type="button" data-bs-target="#tabs-add-expenses">'+
                            '<span>Create Expenses</span>'+
                            '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-plus"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>'+
                        '</button>');

    // Event create by Modal
    $(document).on('click', '.expenses-add', function() {
        var getTarget = $(this).data('bs-target');

        $.get("{{ route('expenses.modal-add') }}", function(data) {
    ...
    // Event Edit by Modal
    $(document).on('click', '.expenses-edit-table', function() {
        var productId = $(this).data('bs-target');
        var parts = productId.split("-");
        var id = parseInt(parts[1]);

        $.get("{{ url('expenses/modal-edit') }}/" + id, function(data) {
            $('#modalContainer').html(data);
    ...
    $(document).on('click', '.expenses-delete-table', function() {
        var productId = $(this).data('bs-target');
        var parts = productId.split("-");
        var id = parseInt(parts[1]);

        $.get("{{ url('expenses/modal-delete') }}/" + id, function(data) {
            $('#modalContainer').html(data);
            $(`${productId}`).modal('show');
        });
    });
