...
class TransactionController extends Controller
{
    ...
    {
        Cart::session(Auth::user()->id)->getContent();
        $other_setting = OtherSetting::first(); // Ambil 1 data saja untuk efisiensi

        // Format ulang data dari request agar tidak error
        $discount_price = (int) str_replace('.', '', $request->discount_price);
        $sub_total = (int) str_replace(['Rp.', '.'], '', $request->sub_total);
        $total = (int) str_replace(['Rp.', '.'], '', $request->total);

        // Pastikan nilai tidak null
        $discount_percent = (int) ($request->discount_percent ?? 0);
        $discount_type = $request->discount_type;
        $sewa = (int) filter_var($request->sewa, FILTER_SANITIZE_NUMBER_INT) ?: 1; // Jika null, set ke 1

        $service = $other_setting->layanan / 100;
        $biaya_layanan = 0;

        // Perhitungan biaya layanan berdasarkan total awal
        if ($other_setting->layanan > 0) {
            $biaya_layanan = max(0, $sub_total * $sewa * $service);
        }

        // Total awal sebelum diskon
        $total_awal = ($sub_total * $sewa) + $biaya_layanan;

        // Perhitungan diskon
        if ($discount_type == 'percent') {
            $discount_amount = $total_awal * $discount_percent / 100;
        } else {
            $discount_amount = $discount_price;
        }

        // Pastikan diskon tidak lebih besar dari total awal
        $discount_amount = min($discount_amount, $total_awal);

        // Hitung total setelah diskon
        $total_after_discount = max(0, $total_awal - $discount_amount);

        // Perhitungan pajak berdasarkan total setelah diskon
        $tax = $total_after_discount * ($other_setting->pb01 / 100);
    ...
    public function returnOrder(Request $request, $id) {
        try {
            ...
            $order->denda_malam = $subtotal * ($request->denda);

            // Hitung kembalian jika metode pembayaran adalah 'Cash'
            if ($request->payment_method == 'Cash' && $request->cash != null) {
                if ($request->denda) {
                    $kembalian = $request->cash - ($subtotal * ($request->denda)) - $dendaBarangRusak;
                }else{
                    $kembalian = $request->cash - $dendaBarangRusak;
                }
            } 
            $order->kembalian = $kembalian ?? 0;
            
            // Hitung total sebelum diskon
            $totalSebelumDiskon = ($subtotal * ($order->sewa + $request->denda)) + $dendaBarangRusak;
            
            // Hitung diskon berdasarkan total sebelum diskon
            if ($order->type_discount == 'percent') {
                $discount = ($totalSebelumDiskon * $order->percent_discount) / 100;
            } else {
                $discount = $order->price_discount;
            }
            
            // Pastikan diskon tidak melebihi total harga
            $finalTotal = max(0, ($totalSebelumDiskon - $discount));
